...
def plot_prosody_model_burn_batch(batch, args: BURNCollatorArgs):
    """
    the batch cotains a dict of measures, each with shape (batch_size, max_len, args.values_per_word)
    we plot each measure as an image, as well as the original audio
    """
    ...
    if batch_size == 1:
        axs = axs.reshape(2, 1)
    for i in range(batch_size):
        axs[0, i].imshow(batch["mpm"][i].T, aspect="auto", interpolation="none")
        axs[0, i].set_title("mpm")
        axs[1, i].imshow(mels[i], aspect="auto", origin="lower", interpolation="none")
        axs[1, i].set_title("audio")
        axs[1, i].set_xticks([])
        axs[1, i].set_yticks([])
...
def plot_prosody_model_ravdess_batch(batch, args: RAVDESSCollatorArgs):
    batch_size = len(batch["audio"])
    fig, axs = plt.subplots(2, batch_size, figsize=(batch_size * 3, 3))
    audios = [a["array"] for a in batch["audio"]]
    srs = [a["sampling_rate"] for a in batch["audio"]]
...
def plot_prosody_model_timit_batch(batch, args: TIMITCollatorArgs):
    batch_size = len(batch["audio"])
